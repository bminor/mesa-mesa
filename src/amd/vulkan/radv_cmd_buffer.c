...
radv_emit_binning_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   radeon_begin(cs);
   radeon_opt_set_context_reg(cs, R_028C44_PA_SC_BINNER_CNTL_0, RADV_TRACKED_PA_SC_BINNER_CNTL_0, pa_sc_binner_cntl_0);
   radeon_end();
}
...
radv_emit_rbplus_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   radeon_begin(cs);
   radeon_opt_set_context_reg3(cs, R_028754_SX_PS_DOWNCONVERT, RADV_TRACKED_SX_PS_DOWNCONVERT, sx_ps_downconvert,
...
radv_emit_ps_epilog_state(struct radv_cmd_buffer *cmd_buffer, struct radv_shader_part *ps_epilog)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      if (pgm_rsrc1)
         gfx12_push_sh_reg(cs, ps_shader->info.regs.pgm_rsrc1, pgm_rsrc1);
      gfx12_push_32bit_pointer(cs, epilog_pc_offset, ps_epilog->va, &pdev->info);
   } else {
      radeon_begin(cs);
      if (pgm_rsrc1)
...
radv_emit_vgt_gs_mode(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   radeon_begin(cs);
   radeon_opt_set_context_reg(cs, R_028A84_VGT_PRIMITIVEID_EN, RADV_TRACKED_VGT_PRIMITIVEID_EN, vgt_primitiveid_en);
   radeon_opt_set_context_reg(cs, R_028A40_VGT_GS_MODE, RADV_TRACKED_VGT_GS_MODE, vgt_gs_mode);
   radeon_end();
}
...
radv_emit_hw_vs(struct radv_cmd_buffer *cmd_buffer, const struct radv_shader *shader)
{
   ...
   radeon_opt_set_context_reg(cs, R_0286C4_SPI_VS_OUT_CONFIG, RADV_TRACKED_SPI_VS_OUT_CONFIG,
                              shader->info.regs.spi_vs_out_config);
   radeon_opt_set_context_reg(cs, R_02870C_SPI_SHADER_POS_FORMAT, RADV_TRACKED_SPI_SHADER_POS_FORMAT,
                              shader->info.regs.spi_shader_pos_format);
   radeon_opt_set_context_reg(cs, R_02881C_PA_CL_VS_OUT_CNTL, RADV_TRACKED_PA_CL_VS_OUT_CNTL,
                              shader->info.regs.pa_cl_vs_out_cntl);

   if (pdev->info.gfx_level <= GFX8)
      radeon_opt_set_context_reg(cs, R_028AB4_VGT_REUSE_OFF, RADV_TRACKED_VGT_REUSE_OFF,
   ...
   if (pdev->info.gfx_level >= GFX7) {
      ...
      if (pdev->info.gfx_level >= GFX10) {
         ...
         if (shader->info.stage == MESA_SHADER_TESS_EVAL) {
            radeon_opt_set_context_reg(cs, R_028A44_VGT_GS_ONCHIP_CNTL, RADV_TRACKED_VGT_GS_ONCHIP_CNTL,
...
radv_emit_hw_ls(struct radv_cmd_buffer *cmd_buffer, const struct radv_shader *shader)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_sh_reg(cs, shader->info.regs.pgm_lo, va >> 8);
      gfx12_push_sh_reg(cs, shader->info.regs.pgm_rsrc1, shader->config.rsrc1);
   } else {
      radeon_begin(cs);
      radeon_set_sh_reg(shader->info.regs.pgm_lo, va >> 8);
...
radv_emit_hw_ngg(struct radv_cmd_buffer *cmd_buffer, const struct radv_shader *es, const struct radv_shader *shader)
{
   ...
   if (!shader->info.merged_shader_compiled_separately) {
      if (pdev->info.gfx_level >= GFX12) {
         gfx12_push_sh_reg(cs, shader->info.regs.pgm_lo, va >> 8);
         gfx12_push_sh_reg(cs, shader->info.regs.pgm_rsrc1, shader->config.rsrc1);
         gfx12_push_sh_reg(cs, shader->info.regs.pgm_rsrc2, shader->config.rsrc2);
         gfx12_push_sh_reg(cs, R_00B220_SPI_SHADER_PGM_RSRC4_GS, shader->info.regs.spi_shader_pgm_rsrc4_gs);
      } else {
         radeon_begin(cs);
         radeon_set_sh_reg(shader->info.regs.pgm_lo, va >> 8);
         radeon_set_sh_reg_seq(shader->info.regs.pgm_rsrc1, 2);
   ...
   if (pdev->info.gfx_level >= GFX12) {
      ...
      gfx12_opt_set_context_reg(cs, R_028818_PA_CL_VS_OUT_CNTL, RADV_TRACKED_PA_CL_VS_OUT_CNTL,
                                shader->info.regs.pa_cl_vs_out_cntl);
      gfx12_opt_set_context_reg(cs, R_028B3C_VGT_GS_INSTANCE_CNT, RADV_TRACKED_VGT_GS_INSTANCE_CNT,
                                shader->info.regs.vgt_gs_instance_cnt);
      gfx12_opt_set_context_reg2(cs, R_028648_SPI_SHADER_IDX_FORMAT, RADV_TRACKED_SPI_SHADER_IDX_FORMAT,
                                 shader->info.regs.ngg.spi_shader_idx_format, shader->info.regs.spi_shader_pos_format);
      gfx12_opt_set_context_reg(cs, R_0287FC_GE_MAX_OUTPUT_PER_SUBGROUP, RADV_TRACKED_GE_MAX_OUTPUT_PER_SUBGROUP,
                                shader->info.regs.ngg.ge_max_output_per_subgroup);
      gfx12_opt_set_context_reg(cs, R_028B4C_GE_NGG_SUBGRP_CNTL, RADV_TRACKED_GE_NGG_SUBGRP_CNTL,
      ...
      radeon_opt_set_context_reg(cs, R_02881C_PA_CL_VS_OUT_CNTL, RADV_TRACKED_PA_CL_VS_OUT_CNTL,
                                 shader->info.regs.pa_cl_vs_out_cntl);
      radeon_opt_set_context_reg(cs, R_028B90_VGT_GS_INSTANCE_CNT, RADV_TRACKED_VGT_GS_INSTANCE_CNT,
                                 shader->info.regs.vgt_gs_instance_cnt);
      radeon_opt_set_context_reg(cs, R_028A84_VGT_PRIMITIVEID_EN, RADV_TRACKED_VGT_PRIMITIVEID_EN,
                                 shader->info.regs.ngg.vgt_primitiveid_en | S_028A84_PRIMITIVEID_EN(es_enable_prim_id));
      radeon_opt_set_context_reg2(cs, R_028708_SPI_SHADER_IDX_FORMAT, RADV_TRACKED_SPI_SHADER_IDX_FORMAT,
                                  shader->info.regs.ngg.spi_shader_idx_format, shader->info.regs.spi_shader_pos_format);
      radeon_opt_set_context_reg(cs, R_0286C4_SPI_VS_OUT_CONFIG, RADV_TRACKED_SPI_VS_OUT_CONFIG,
                                 shader->info.regs.spi_vs_out_config);
      radeon_opt_set_context_reg(cs, R_0287FC_GE_MAX_OUTPUT_PER_SUBGROUP, RADV_TRACKED_GE_MAX_OUTPUT_PER_SUBGROUP,
                                 shader->info.regs.ngg.ge_max_output_per_subgroup);
      radeon_opt_set_context_reg(cs, R_028B4C_GE_NGG_SUBGRP_CNTL, RADV_TRACKED_GE_NGG_SUBGRP_CNTL,
                                 shader->info.regs.ngg.ge_ngg_subgrp_cntl);
      radeon_end();
   }
...
radv_emit_vertex_shader(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (vs->info.merged_shader_compiled_separately) {
      ...
      if (pdev->info.gfx_level >= GFX12) {
         gfx12_push_32bit_pointer(cs, next_stage_pc_offset, next_stage->va, &pdev->info);

         if (!vs->info.vs.has_prolog) {
            gfx12_push_sh_reg(cs, vs->info.regs.pgm_lo, vs->va >> 8);
            if (vs->info.next_stage == MESA_SHADER_TESS_CTRL) {
               gfx12_push_sh_reg(cs, vs->info.regs.pgm_rsrc1, rsrc1);
            } else {
               gfx12_push_sh_reg(cs, vs->info.regs.pgm_rsrc1, rsrc1);
               gfx12_push_sh_reg(cs, vs->info.regs.pgm_rsrc2, rsrc2);
            }
         }
      } else {
         radeon_begin(cs);
...
radv_emit_tess_eval_shader(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (tes->info.merged_shader_compiled_separately) {
      ...
      if (pdev->info.gfx_level >= GFX12) {
         gfx12_push_sh_reg(cs, tes->info.regs.pgm_lo, tes->va >> 8);
         gfx12_push_sh_reg(cs, tes->info.regs.pgm_rsrc1, rsrc1);
         gfx12_push_sh_reg(cs, tes->info.regs.pgm_rsrc2, rsrc2);
         gfx12_push_32bit_pointer(cs, next_stage_pc_offset, gs->va, &pdev->info);
      } else {
         radeon_begin(cs);
...
radv_emit_hw_gs(struct radv_cmd_buffer *cmd_buffer, const struct radv_shader *gs)
{
   ...
   radeon_opt_set_context_reg3(cs, R_028A60_VGT_GSVS_RING_OFFSET_1, RADV_TRACKED_VGT_GSVS_RING_OFFSET_1,
                               gs->info.regs.gs.vgt_gsvs_ring_offset[0], gs->info.regs.gs.vgt_gsvs_ring_offset[1],
                               gs->info.regs.gs.vgt_gsvs_ring_offset[2]);

   radeon_opt_set_context_reg(cs, R_028AB0_VGT_GSVS_RING_ITEMSIZE, RADV_TRACKED_VGT_GSVS_RING_ITEMSIZE,
                              gs->info.regs.gs.vgt_gsvs_ring_itemsize);

   radeon_opt_set_context_reg4(cs, R_028B5C_VGT_GS_VERT_ITEMSIZE, RADV_TRACKED_VGT_GS_VERT_ITEMSIZE,
   ...
   if (pdev->info.gfx_level >= GFX9) {
      ...
      if (pdev->info.gfx_level == GFX9) {
         radeon_opt_set_context_reg(cs, R_028A94_VGT_GS_MAX_PRIMS_PER_SUBGROUP,
       ...
       */
      radeon_opt_set_context_reg(cs, R_028AAC_VGT_ESGS_RING_ITEMSIZE, RADV_TRACKED_VGT_ESGS_RING_ITEMSIZE,
...
radv_emit_geometry_shader(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   radeon_opt_set_context_reg(cs, R_028B38_VGT_GS_MAX_VERT_OUT, RADV_TRACKED_VGT_GS_MAX_VERT_OUT,
...
radv_emit_vgt_gs_out(struct radv_cmd_buffer *cmd_buffer, uint32_t vgt_gs_out_prim_type)
{
   ...
   if (pdev->info.gfx_level >= GFX11) {
      ...
      radeon_opt_set_context_reg(cs, R_028A6C_VGT_GS_OUT_PRIM_TYPE, RADV_TRACKED_VGT_GS_OUT_PRIM_TYPE,
...
radv_gfx11_emit_meshlet(struct radv_cmd_buffer *cmd_buffer, const struct radv_shader *ms)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_sh_reg(cs, R_00B2B0_SPI_SHADER_GS_MESHLET_DIM, ms->info.regs.ms.spi_shader_gs_meshlet_dim);
      gfx12_push_sh_reg(cs, R_00B2B4_SPI_SHADER_GS_MESHLET_EXP_ALLOC, ms->info.regs.ms.spi_shader_gs_meshlet_exp_alloc);
      gfx12_push_sh_reg(cs, R_00B2B8_SPI_SHADER_GS_MESHLET_CTRL, ms->info.regs.ms.spi_shader_gs_meshlet_ctrl);
   } else {
      radeon_begin(cs);
...
radv_emit_ps_inputs(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      radeon_opt_set_context_regn(cs, R_028664_SPI_PS_INPUT_CNTL_0, ps_input_cntl, cs->tracked_regs.spi_ps_input_cntl,
                                  ps_offset);
   } else {
      if (pdev->info.gfx_level == GFX10_3) {
         ...
         radeon_opt_set_context_reg(cs, R_0286D8_SPI_PS_IN_CONTROL, RADV_TRACKED_SPI_PS_IN_CONTROL,
      ...
      radeon_opt_set_context_regn(cs, R_028644_SPI_PS_INPUT_CNTL_0, ps_input_cntl, cs->tracked_regs.spi_ps_input_cntl,
                                  ps_offset);
   }
...
radv_emit_fragment_shader_state(struct radv_cmd_buffer *cmd_buffer, const struct radv_shader *ps)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      ...
      radeon_begin(cs);
      gfx12_begin_context_regs();
      gfx12_opt_set_context_reg2(cs, R_02865C_SPI_PS_INPUT_ENA, RADV_TRACKED_SPI_PS_INPUT_ENA, spi_ps_input_ena,
                                 spi_ps_input_addr);

      gfx12_opt_set_context_reg(cs, R_028640_SPI_PS_IN_CONTROL, RADV_TRACKED_SPI_PS_IN_CONTROL, spi_ps_in_control);

      gfx12_opt_set_context_reg(cs, R_028BBC_PA_SC_HISZ_CONTROL, RADV_TRACKED_PA_SC_HISZ_CONTROL, pa_sc_hisz_control);
      gfx12_end_context_regs();
      radeon_end();
   } else {
      ...
      radeon_begin(cs);
      radeon_opt_set_context_reg2(cs, R_0286CC_SPI_PS_INPUT_ENA, RADV_TRACKED_SPI_PS_INPUT_ENA, spi_ps_input_ena,
                                  spi_ps_input_addr);

      if (pdev->info.gfx_level != GFX10_3) {
         radeon_opt_set_context_reg(cs, R_0286D8_SPI_PS_IN_CONTROL, RADV_TRACKED_SPI_PS_IN_CONTROL, spi_ps_in_control);
      }

      if (pdev->info.gfx_level >= GFX9 && pdev->info.gfx_level < GFX11)
         radeon_opt_set_context_reg(cs, R_028C40_PA_SC_SHADER_CONTROL, RADV_TRACKED_PA_SC_SHADER_CONTROL,
...
radv_emit_fragment_shader(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_sh_reg(cs, ps->info.regs.pgm_lo, va >> 8);
      gfx12_push_sh_reg(cs, ps->info.regs.pgm_rsrc1, ps->config.rsrc1);
      gfx12_push_sh_reg(cs, ps->info.regs.pgm_rsrc2, ps->config.rsrc2);
   } else {
      radeon_begin(cs);
...
radv_emit_vgt_reuse(struct radv_cmd_buffer *cmd_buffer, const struct radv_vgt_shader_key *key)
{
   ...
   if (pdev->info.family >= CHIP_POLARIS10 && pdev->info.gfx_level < GFX10) {
      ...
      radeon_begin(cs);
      radeon_opt_set_context_reg(cs, R_028C58_VGT_VERTEX_REUSE_BLOCK_CNTL, RADV_TRACKED_VGT_VERTEX_REUSE_BLOCK_CNTL,
...
radv_emit_vgt_shader_config_gfx12(struct radv_cmd_buffer *cmd_buffer, const struct radv_vgt_shader_key *key)
{
   ...
   radeon_begin(cs);
   radeon_opt_set_context_reg(cs, R_028A98_VGT_SHADER_STAGES_EN, RADV_TRACKED_VGT_SHADER_STAGES_EN, stages);
   radeon_end();
}
...
radv_emit_vgt_shader_config_gfx6(struct radv_cmd_buffer *cmd_buffer, const struct radv_vgt_shader_key *key)
{
   ...
   radeon_begin(cs);
   radeon_opt_set_context_reg(cs, R_028B54_VGT_SHADER_STAGES_EN, RADV_TRACKED_VGT_SHADER_STAGES_EN, stages);
   radeon_end();
}
...
gfx103_emit_vgt_draw_payload_cntl(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      radeon_opt_set_context_reg(cs, R_028AA0_VGT_DRAW_PAYLOAD_CNTL, RADV_TRACKED_VGT_DRAW_PAYLOAD_CNTL,
                                 vgt_draw_payload_cntl);
   } else {
      radeon_opt_set_context_reg(cs, R_028A98_VGT_DRAW_PAYLOAD_CNTL, RADV_TRACKED_VGT_DRAW_PAYLOAD_CNTL,
                                 vgt_draw_payload_cntl);
   }
...
gfx103_emit_vrs_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (enable_vrs_coarse_shading) {
       ...
       */
      radeon_begin(cs);
      radeon_opt_set_context_reg(cs, R_028848_PA_CL_VRS_CNTL, RADV_TRACKED_PA_CL_VRS_CNTL,
   ...
   if (pdev->info.gfx_level < GFX11) {
      radeon_begin(cs);
      radeon_opt_set_context_reg(cs, R_028064_DB_VRS_OVERRIDE_CNTL, RADV_TRACKED_DB_VRS_OVERRIDE_CNTL,
...
radv_emit_graphics_shaders(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      ...
      gfx12_push_sh_reg(cmd_buffer->cs, R_00B0C4_SPI_SHADER_GS_OUT_CONFIG_PS, gs_out_config_ps);
   }
...
radv_flush_occlusion_query_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028060_DB_COUNT_CONTROL, RADV_TRACKED_DB_COUNT_CONTROL,
                                 db_count_control);
   } else {
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028004_DB_COUNT_CONTROL, RADV_TRACKED_DB_COUNT_CONTROL,
                                 db_count_control);
   }
                 ...
                 const struct radv_shader_part *prolog)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_sh_reg(cs, vs_shader->info.regs.pgm_lo, prolog->va >> 8);
      gfx12_push_sh_reg(cs, vs_shader->info.regs.pgm_rsrc1, rsrc1);
      if (vs_shader->info.merged_shader_compiled_separately)
         gfx12_push_sh_reg(cs, vs_shader->info.regs.pgm_rsrc2, rsrc2);
   } else {
      radeon_begin(cs);
...
radv_emit_streamout_buffers(struct radv_cmd_buffer *cmd_buffer, uint64_t va)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_32bit_pointer(cmd_buffer->cs, streamout_buffers_offset, va, &pdev->info);
   } else {
      radeon_begin(cmd_buffer->cs);
...
radv_emit_streamout_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   gfx12_push_32bit_pointer(cmd_buffer->cs, streamout_state_offset, so->state_va, &pdev->info);
}
...
radv_flush_force_vrs_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (cmd_buffer->state.last_vrs_rates != vrs_rates ||
      ...
      if (pdev->info.gfx_level >= GFX12) {
         gfx12_push_sh_reg(cmd_buffer->cs, force_vrs_rates_offset, vrs_rates);
      } else {
         radeon_begin(cmd_buffer->cs);
...
radv_emit_primitive_restart(struct radv_cmd_buffer *cmd_buffer, bool enable)
{
   ...
   if (gfx_level >= GFX11) {
      ...
      if (enable && gfx_level <= GFX7) {
         ...
         radeon_opt_set_context_reg(cs, R_02840C_VGT_MULTI_PRIM_IB_RESET_INDX,
...
radv_emit_fs_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_sh_reg(cmd_buffer->cs, ps_state_offset, ps_state);
   } else {
      radeon_begin(cmd_buffer->cs);
...
radv_emit_ngg_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_sh_reg(cmd_buffer->cs, ngg_state_offset, ngg_state);
      if (ngg_query_buf_va_offset)
         gfx12_push_sh_reg(cmd_buffer->cs, ngg_query_buf_va_offset, cmd_buffer->state.shader_query_buf_va);
   } else {
      radeon_begin(cmd_buffer->cs);
...
radv_emit_tess_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      gfx12_push_sh_reg(cs, tcs->info.regs.pgm_rsrc2, pgm_hs_rsrc2);
      if (tcs_offchip_layout || tes_offchip_layout) {
         gfx12_push_sh_reg(cs, tcs_offchip_layout_offset, tcs_offchip_layout);
         gfx12_push_sh_reg(cs, tes_offchip_layout_offset, tes_offchip_layout);
      }
   } else {
      radeon_begin(cs);
...
radv_emit_db_shader_control(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      radeon_opt_set_context_reg(cmd_buffer->cs, R_02806C_DB_SHADER_CONTROL, RADV_TRACKED_DB_SHADER_CONTROL,
                                 db_shader_control);
   } else {
      radeon_opt_set_context_reg(cmd_buffer->cs, R_02880C_DB_SHADER_CONTROL, RADV_TRACKED_DB_SHADER_CONTROL,
                                 db_shader_control);
...
radv_emit_fragment_output_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      ...
      radeon_begin(cmd_buffer->cs);
      radeon_opt_set_context_reg(cmd_buffer->cs, R_02823C_CB_SHADER_MASK, RADV_TRACKED_CB_SHADER_MASK,
                                 cmd_buffer->state.cb_shader_mask);
      radeon_opt_set_context_reg2(cmd_buffer->cs, R_028710_SPI_SHADER_Z_FORMAT, RADV_TRACKED_SPI_SHADER_Z_FORMAT,
...
radv_emit_depth_stencil_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      ...
      radeon_begin(cmd_buffer->cs);
      gfx12_begin_context_regs();
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_02800C_DB_RENDER_OVERRIDE, RADV_TRACKED_DB_RENDER_OVERRIDE,
                                S_02800C_FORCE_STENCIL_READ(1) | S_02800C_FORCE_STENCIL_VALID(force_s_valid));

      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028070_DB_DEPTH_CONTROL, RADV_TRACKED_DB_DEPTH_CONTROL,
                                db_depth_control);

      if (ds.stencil.test_enable) {
         gfx12_opt_set_context_reg(cmd_buffer->cs, R_028074_DB_STENCIL_CONTROL, RADV_TRACKED_DB_STENCIL_CONTROL,
                                   db_stencil_control);

         gfx12_opt_set_context_reg(
            cmd_buffer->cs, R_028088_DB_STENCIL_REF, RADV_TRACKED_DB_STENCIL_REF,
            S_028088_TESTVAL(ds.stencil.front.reference) | S_028088_TESTVAL_BF(ds.stencil.back.reference));

         gfx12_opt_set_context_reg2(
            cmd_buffer->cs, R_028090_DB_STENCIL_READ_MASK, RADV_TRACKED_DB_STENCIL_READ_MASK,
      ...
      if (ds.depth.bounds_test.enable) {
         gfx12_opt_set_context_reg2(cmd_buffer->cs, R_028050_DB_DEPTH_BOUNDS_MIN, RADV_TRACKED_DB_DEPTH_BOUNDS_MIN,
      ...
      radeon_begin(cmd_buffer->cs);
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028800_DB_DEPTH_CONTROL, RADV_TRACKED_DB_DEPTH_CONTROL,
                                 db_depth_control);

      if (ds.stencil.test_enable) {
         radeon_opt_set_context_reg(cmd_buffer->cs, R_02842C_DB_STENCIL_CONTROL, RADV_TRACKED_DB_STENCIL_CONTROL,
                                    db_stencil_control);

         radeon_opt_set_context_reg2(
            cmd_buffer->cs, R_028430_DB_STENCILREFMASK, RADV_TRACKED_DB_STENCILREFMASK,
      ...
      if (ds.depth.bounds_test.enable) {
         radeon_opt_set_context_reg2(cmd_buffer->cs, R_028020_DB_DEPTH_BOUNDS_MIN, RADV_TRACKED_DB_DEPTH_BOUNDS_MIN,
                                     depth_bounds_min, depth_bounds_max);
      }
      radeon_end();
   }
...
radv_emit_raster_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      radeon_begin(cmd_buffer->cs);
      gfx12_begin_context_regs();
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028A08_PA_SU_LINE_CNTL, RADV_TRACKED_PA_SU_LINE_CNTL,
                                pa_su_line_cntl);

      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028A0C_PA_SC_LINE_STIPPLE, RADV_TRACKED_PA_SC_LINE_STIPPLE,
      ...
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028BDC_PA_SC_LINE_CNTL, RADV_TRACKED_PA_SC_LINE_CNTL,
                                pa_sc_line_cntl);

      gfx12_opt_set_context_reg(
         cmd_buffer->cs, R_028810_PA_CL_CLIP_CNTL, RADV_TRACKED_PA_CL_CLIP_CNTL,
      ...
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028A44_PA_SC_LINE_STIPPLE_RESET,
                                RADV_TRACKED_PA_SC_LINE_STIPPLE_RESET, S_028A44_AUTO_RESET_CNTL(auto_reset_cntl));

      gfx12_opt_set_context_reg(cmd_buffer->cs, R_02881C_PA_SU_SC_MODE_CNTL, RADV_TRACKED_PA_SU_SC_MODE_CNTL,
                                pa_su_sc_mode_cntl);
      gfx12_end_context_regs();
      radeon_end();
   } else {
      radeon_begin(cmd_buffer->cs);
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028A08_PA_SU_LINE_CNTL, RADV_TRACKED_PA_SU_LINE_CNTL,
                                 pa_su_line_cntl);

      radeon_opt_set_context_reg(cmd_buffer->cs, R_028A0C_PA_SC_LINE_STIPPLE, RADV_TRACKED_PA_SC_LINE_STIPPLE,
                                 S_028A0C_LINE_PATTERN(d->vk.rs.line.stipple.pattern) |
                                    S_028A0C_REPEAT_COUNT(d->vk.rs.line.stipple.factor - 1) |
                                    S_028A0C_AUTO_RESET_CNTL(pdev->info.gfx_level < GFX12 ? auto_reset_cntl : 0));

      radeon_opt_set_context_reg(cmd_buffer->cs, R_028BDC_PA_SC_LINE_CNTL, RADV_TRACKED_PA_SC_LINE_CNTL,
                                 pa_sc_line_cntl);
      ...
      radeon_opt_set_context_reg(
         ...
         S_028810_DX_RASTERIZATION_KILL(d->vk.rs.rasterizer_discard_enable) |
            S_028810_ZCLIP_NEAR_DISABLE(!depth_clip_enable) | S_028810_ZCLIP_FAR_DISABLE(!depth_clip_enable) |
            S_028810_DX_CLIP_SPACE_DEF(!d->vk.vp.depth_clip_negative_one_to_one) | S_028810_DX_LINEAR_ATTR_CLIP_ENA(1));
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028814_PA_SU_SC_MODE_CNTL, RADV_TRACKED_PA_SU_SC_MODE_CNTL,
                                 pa_su_sc_mode_cntl);
      radeon_end();
   }
...
radv_emit_cb_render_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   radeon_begin(cmd_buffer->cs);
   radeon_opt_set_context_regn(cmd_buffer->cs, R_028780_CB_BLEND0_CONTROL, cb_blend_control,
                               cmd_buffer->cs->tracked_regs.cb_blend_control, MAX_RTS);
   if (pdev->info.has_rbplus) {
      radeon_opt_set_context_regn(cmd_buffer->cs, R_028760_SX_MRT0_BLEND_OPT, sx_mrt_blend_opt,
                                  cmd_buffer->cs->tracked_regs.sx_mrt_blend_opt, MAX_RTS);
   }

   if (pdev->info.gfx_level >= GFX12) {
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028850_CB_TARGET_MASK, RADV_TRACKED_CB_TARGET_MASK, cb_target_mask);
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028858_CB_COLOR_CONTROL, RADV_TRACKED_CB_COLOR_CONTROL,
                                 cb_color_control);
   } else {
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028238_CB_TARGET_MASK, RADV_TRACKED_CB_TARGET_MASK, cb_target_mask);
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028808_CB_COLOR_CONTROL, RADV_TRACKED_CB_COLOR_CONTROL,
                                 cb_color_control);
   }
   radeon_end();
...
radv_emit_msaa_state(struct radv_cmd_buffer *cmd_buffer)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      radeon_begin(cmd_buffer->cs);
      gfx12_begin_context_regs();
      gfx12_opt_set_context_reg2(cmd_buffer->cs, R_028C38_PA_SC_AA_MASK_X0Y0_X1Y0, RADV_TRACKED_PA_SC_AA_MASK_X0Y0_X1Y0,
                                 sample_mask, sample_mask);
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028BE0_PA_SC_AA_CONFIG, RADV_TRACKED_PA_SC_AA_CONFIG,
                                pa_sc_aa_config);
      gfx12_opt_set_context_reg(
         cmd_buffer->cs, R_028A48_PA_SC_MODE_CNTL_0, RADV_TRACKED_PA_SC_MODE_CNTL_0,
      ...
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_02807C_DB_ALPHA_TO_MASK, RADV_TRACKED_DB_ALPHA_TO_MASK,
                                db_alpha_to_mask);
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028C5C_PA_SC_SAMPLE_PROPERTIES, RADV_TRACKED_PA_SC_SAMPLE_PROPERTIES,
                                S_028C5C_MAX_SAMPLE_DIST(max_sample_dist));
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028078_DB_EQAA, RADV_TRACKED_DB_EQAA, db_eqaa);
      gfx12_opt_set_context_reg(cmd_buffer->cs, R_028C54_PA_SC_CONSERVATIVE_RASTERIZATION_CNTL,
                                RADV_TRACKED_PA_SC_CONSERVATIVE_RASTERIZATION_CNTL, pa_sc_conservative_rast);
      gfx12_end_context_regs();
      radeon_end();
   } else {
      radeon_begin(cmd_buffer->cs);
      radeon_opt_set_context_reg2(cmd_buffer->cs, R_028C38_PA_SC_AA_MASK_X0Y0_X1Y0,
                                  RADV_TRACKED_PA_SC_AA_MASK_X0Y0_X1Y0, sample_mask, sample_mask);
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028BE0_PA_SC_AA_CONFIG, RADV_TRACKED_PA_SC_AA_CONFIG,
                                 pa_sc_aa_config);
      radeon_opt_set_context_reg(
         cmd_buffer->cs, R_028A48_PA_SC_MODE_CNTL_0, RADV_TRACKED_PA_SC_MODE_CNTL_0,
         S_028A48_ALTERNATE_RBS_PER_TILE(pdev->info.gfx_level >= GFX9) | S_028A48_VPORT_SCISSOR_ENABLE(1) |
            S_028A48_LINE_STIPPLE_ENABLE(d->vk.rs.line.stipple.enable) | S_028A48_MSAA_ENABLE(msaa_enable));
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028B70_DB_ALPHA_TO_MASK, RADV_TRACKED_DB_ALPHA_TO_MASK,
                                 db_alpha_to_mask);
      radeon_opt_set_context_reg(cmd_buffer->cs, R_028804_DB_EQAA, RADV_TRACKED_DB_EQAA, db_eqaa);

      if (pdev->info.gfx_level >= GFX9)
         radeon_opt_set_context_reg(cmd_buffer->cs, R_028C4C_PA_SC_CONSERVATIVE_RASTERIZATION_CNTL,
                                    RADV_TRACKED_PA_SC_CONSERVATIVE_RASTERIZATION_CNTL, pa_sc_conservative_rast);
      radeon_end();
   }
...
radv_before_draw(struct radv_cmd_buffer *cmd_buffer, const struct radv_draw_info *info, uint32_t drawCount, bool dgc)
{
   ...
   if (pdev->info.gfx_level >= GFX12) {
      radeon_begin(cs);
      gfx12_emit_buffered_sh_regs(cs);
      radeon_end();
   }
